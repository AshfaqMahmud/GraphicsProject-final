...
struct Material {
    sampler2D diffuse;  //texture type which is int
    sampler2D specular;
    float shininess;
};
...
vec3 CalcPointLight(Material material, PointLight light, vec3 N, vec3 fragPos, vec3 V)
{
    ...
    vec3 ambient = vec3(texture(material.diffuse, TexCoords)) * light.ambient;  //property taken from texture instead of material
